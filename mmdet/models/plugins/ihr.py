...
class IHR(nn.Module):
    ...
    def __init__(self,
        ...
        in_channel = metric_module['in_channel']
        out_channel = metric_module['out_channel']
    ...
    def forward(self, x, ref):    
        ...
        key_t=self.key_t(x)
        p = torch.matmul(key_t.view(bs,16,-1).permute(0,2,1),key_q.view(bs,16,-1))
        p = F.softmax(p,dim=1)
        val_t_out = torch.matmul(ref.view(bs,128,-1),p).view(bs,128,ref_h,ref_w)
